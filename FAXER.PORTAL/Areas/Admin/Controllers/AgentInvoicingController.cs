...
{
    public class AgentInvoicingController : Controller
    {
        ViewAlertsServices Service = new ViewAlertsServices();
        CommonServices _commonServices = new CommonServices();
        // GET: Admin/AgentInvoicing
        public ActionResult Index(string CountryCode = "", string City = "", int AgentId = 0, string Date = "", string message = "")
        {
        ...
        [HttpGet]
        public ActionResult CreateInvoice(int Id = 0, string Country = "", string City = "")
        {
            SetViewBagForCountries();
        ...
        [HttpPost]
        public ActionResult CreateInvoice([Bind(Include = CreateAgentInvoicingViewModel.BindProperty)]CreateAgentInvoicingViewModel model)
        {
            SetViewBagForCountries();
            SetViewBagForCities(model.Country);
            SetViewBagForAgents(model.Country, model.City);
            CreateAgentInvoicingViewModel vm = new CreateAgentInvoicingViewModel();
            return RedirectToAction("AgentPayAnInvoiceSuccess",vm);
        }

        public ActionResult AgentPayAnInvoiceSuccess()
        {
