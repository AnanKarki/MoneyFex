var Riddha = {
    ...
    ko: {
        global: {
            ...
            filter: function (array, field, value) {
                var maped = ko.utils.arrayFilter(array(), function (item) {
                    return item[field]() == value();
                });
                return maped;
            },
            sum: function (array, fieldName) {
                var sum = 0;
                for (var i = 0; i < array.length; i++) {
                    sum += parseFloat( array[i][fieldName]);
                }
            }
    ...
    util:
    {
        ...
        localize: {
            ...
            return dayName;
        }, getShortDayName: function (dateString) {
            var days = Riddha.config().CurrentLanguage == "ne" ? ['आइत', 'सोम', 'मंगल', 'बुध', 'बिही', 'शुक्र', 'शनि'] : ['Sun', 'Mon', 'Tue', 'Wed', 'Thur', 'Fri', 'Sat'];
            var d = new Date(dateString);
            var dayName = days[d.getDay()];
            return dayName;
        },
